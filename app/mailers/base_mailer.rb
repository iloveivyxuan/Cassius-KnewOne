class BaseMailer < ActionMailer::Base
  ...
  def mail(headers = {}, &block)
    ...
    if self.delivery_method == :smtp
      smtp_config = route_smtp_server(headers[:to], headers.delete(:edm))
  ...
  def route_smtp_server(email, edm = nil)
    if edm
      EDM_SMTP_SERVERS[(WHITES.include?(/.*@(.+)/.match(email)[1]) ? :white : :default)]
    else
